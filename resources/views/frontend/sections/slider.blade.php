...
<section class="mb-2">
    <div class="owl-carousel owl-theme" id="mainSliderOwl">
        ...
        {{-- <div class="item">
            ...
            <div
                class="main-slider-caption-right d-flex flex-column justify-content-center align-items-end text-end p-4 position-absolute top-0 end-0 h-100 w-100">
        ...
        @foreach ($sliders as $slider)
            <div class="item">
                <img src="{{ $slider->getFirstMediaUrl('image') }}" alt="{{ $slider->libelle }}">
                <div
                    class="main-slider-caption-right d-flex flex-column justify-content-center align-items-end text-end p-4 position-absolute top-0 end-0 h-100 w-100">
                    <h1 class="display-5 fw-bold text-white">{{ $slider->libelle }}</h1>
                    <p class="mb-1 text-white">{{ $slider->description ?? 'Le vrai goût du poulet' }}</p>
                   
                        <a href="{{ $slider->url ?? route('boutique.index') }}" class="btn btn-cta btn-lg mt-3">
                            {{ $slider->btn_nom ?? 'Commander maintenant' }}</a>
