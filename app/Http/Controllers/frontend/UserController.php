...
class UserController extends Controller
{
    ...
    {
        // Sauvegarder l'URL précédente si elle existe sauf login/register
    ...
    public function login(Request $request)
    {
        ...
        if ($this->isSha1($user->password)) {
            ...
            if (sha1($password) === $user->password) {
                Auth::login($user);
                $request->session()->regenerate();// Sécurité de session
                // Ré-hashage en Bcrypt
                $user->password = Hash::make($password);
                $user->save();

                return redirect() ->intended(route('boutique.accueil'))->with('success', 'Connexion réussie (mot de passe mis à jour) !');
            }
        } else {
            // Bcrypt classique
            if (Hash::check($password, $user->password)) {
                ...
                return redirect()->intended(route('boutique.accueil'))->with('success', 'Connexion réussie !');
            }
        }
    ...
    public function register(Request $request)
    {
        ...
        try {
            ...
            return redirect()->intended(route('boutique.accueil'))->with('success', 'Inscription réussie !');
        } catch (\Throwable $th) {
